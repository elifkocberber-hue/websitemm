...
import type { CeramicProduct } from '@/types/ceramic';

// Ana sayfa (TR ve /en) için ortak veri çekme. İki sayfa da aynı DB içeriğini
// gösterir; dil farkı yalnız client provider'da (statik UI çevirileri).
...
// Ana sayfanın ihtiyaç duyduğu tüm veriyi tek çağrıda hazırlar.
export async function getHomeData() {
  ...
  let featured: CeramicProduct[] = allProducts.slice(0, 4);
  if (featuredIds.length > 0) {
    const picked = featuredIds
      .map((id) => allProducts.find((p) => String(p.id) === String(id)))
      .filter(Boolean) as CeramicProduct[];
    if (picked.length > 0) {
  ...
  return { featured, banner, about, homepage };
}
